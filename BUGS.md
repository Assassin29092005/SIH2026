...
### BUG-028 — `ground_gsd()` took a median over a quantised backplane and read 18.65% high
...
- **Fix:** `scripts/iirs_vs_m3.py` — `ground_gsd` now takes the end-to-end swath width divided by its interval count, per line, then averages: duplicate-proof by construction, because the zeros cancel on any end-to-end measure of collinear samples. The haversine moved into `_arc_m` so both the measure and the check can use it. Verified on the real IIRS backplane: **84.48 m**, against the 84.48 the sum-of-steps route gives and the 100.24 the median gave. `outputs/iirs_vs_m3.json` regenerated.
...
- **Check:** `python scripts/iirs_vs_m3.py --self-check` — needs no data. It builds a coordinate line at a known spacing and **quantises** it, which is the actual mechanism: the swath width is fixed by the optics and what the backplane loses is coordinate precision, so rounding creates duplicates *and* inflates every surviving step. A fixture that merely drops samples shortens the swath and reproduces nothing — the first attempt did exactly that and had to be rebuilt. On the fixture: 37% duplicated samples, end-to-end exact, old median **+59.6%**. The check asserts both that the new measure is right and that the old one would have been wrong, so it fails if the fixture ever stops reproducing the bug.

### BUG-027 — the one-command demo reported TMC-2 as NOT sub-pixel while the README reported it as sub-pixel
...
- **Fix:** `scripts/demo.py` now calls `sandhi.pipeline.register()` and `pipeline.warp()`, and takes its metric block from the package instead of recomputing one. All three committed figures and their JSONs were regenerated on full data, and the gate that guards the package path — `test_control_gate_passes`, parametrised over all three cases — passes.

  Two traps had to be avoided, both of which would have made the fix worse than the bug. The loaders already resample to the common grid, so `run()` passes `gsd_m` and `source_native_gsd` but **not** `src_gsd`/`ref_gsd`, which would have resampled a second time. And `source_native_gsd` is what keeps OHRC honest: without it the 0.26 m product is scored against the 7.403 m grid it was projected onto, which flips it from a stated limit to a false SUB-PIXEL pass and deletes exactly the caveat the demo exists to show.

  After, on full data: TMC-2 reports 3085 matches, 3064 inliers, `rmse_px` 0.5950404402459971 and `rmse_source_px` 0.8722939364635874 — **bit-identical to `outputs/tmc2_metrics.json` across all nine compared fields**, model `affine`. OHRC keeps its limit at 14.618 source px, `sub_pixel_source: false`. Kaguya is unchanged at 0.796 px.
- **Check:** `tests/test_units.py::test_demo_and_reported_metrics_agree_on_the_subpixel_verdict` compares `outputs/demo_tmc.json` against `outputs/tmc2_metrics.json` on `sub_pixel_source` and `sub_pixel` — a **verdict-level** comparison, so it pins neither path's numbers and survives a re-measurement of either. Verified load-bearing: flipping `sub_pixel_source` in the committed demo JSON fails it. `tests/test_units.py::test_every_demo_metrics_file_states_the_source_pixel_figure` additionally asserts every committed demo result carries the source-pixel figure and that it equals the grid figure times the scale ratio, so the two can never drift apart silently.
